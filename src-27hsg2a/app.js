...
bts.factory('Common', ['$location', '$http', 'ConfigSvc', function ($location, $http, ConfigSvc) {
  ...
  var isMobile = function () {
    ...
    Logger.info('bts.Common.isMobile: ' + check + ' ' + navigator.userAgent);
    return check;
  };
  var isMobileTabletDevice = function  () {
    var retVal = false;
    ...
    Logger.info('bts.Common.isMobileTabletDevice: ' + retVal + ' ' + navigator.userAgent);
    return true;
  };
  var isMobileOverride = ($location.search()).mobile;
  Logger.debug('bts.Common.isMobileOverride: ' + isMobileOverride);
  c.isMobile = isMobile();
  c.isMobileDevice = isMobile();
  c.isMobileTabletDevice = isMobileTabletDevice();
  ...
  Logger.info('bts.Common.isMobile (final): ' + c.isMobile + ' type: ' + typeof(c.isMobile));
...
bts.factory('DoNotReloadCurrentTemplate', ['$route', function($route) {
  return function(scope) {
    Logger.info('DoNotReloadCurrentTemplate');
    var lastRoute = $route.current;
    scope.$on('$locationChangeSuccess', function() {
      if (lastRoute.$$route.templateUrl === $route.current.$$route.templateUrl) {
        Logger.debug('DoNotReloadCurrentTemplate not reloading template: ' + $route.current.$$route.templateUrl);
